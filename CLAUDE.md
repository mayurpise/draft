...
- **`index.md`** — Open Knowledge Format bundle root (`type: Repository`); cross-links every concept below so the whole `draft/` tree is a portable OKF bundle. Generated by default via `scripts/tools/okf-bundle.sh`

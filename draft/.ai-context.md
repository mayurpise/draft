...
- **Language**: Markdown (skill definitions, ~12,000 lines), Bash (build/test, ~776 lines)
...
│   └── templates/        ← 14 output structure templates for user's draft/ directory
├── scripts/
│   └── build-integrations.sh ← Generates Copilot + Gemini integrations (656 lines)
├── tests/
...
- [Data] **Frontmatter required**: Every SKILL.md must have `name:` and `description:` in YAML frontmatter — enforced at `build-integrations.sh:134-142`
- [Data] **Body format**: After frontmatter: blank line, `# Title`, blank line — enforced at `build-integrations.sh:488-501`
...
- [Build] **No /draft: in output**: Integration files must use platform syntax — enforced at `build-integrations.sh:580-589`
- [Build] **No @draft in Copilot**: Copilot uses bare `draft ` — enforced at `build-integrations.sh:591-602`
- [Build] **Skill name regex**: `[a-z0-9-]+` only, prevents path traversal — enforced at `build-integrations.sh:474-477`
...
**Init Flow**: User invokes /draft:init → 5-phase codebase analysis (Discovery, Wiring, Depth, Periphery, Synthesis) → generates draft/architecture.md (comprehensive) → Condensation Subroutine → generates draft/.ai-context.md (200-400 lines) → product/tech-stack/workflow dialogue → writes all draft/ files.
...
3. Add to `SKILL_ORDER` array in `build-integrations.sh:30-47`
4. Add to `get_skill_header()` case at `build-integrations.sh:50-70`
5. Add to `get_trigger()` case at `build-integrations.sh:75-98`
...
- **NOT tested**: skill content correctness

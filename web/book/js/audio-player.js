...
// Map chapter slug (from body[data-chapter]) to audio file index
const CHAPTER_AUDIO_MAP = {
    ...
    'incremental-refresh': 13,
    'deep-review': 14,
    'bug-hunt': 15,
    'pattern-learning': 17,
    'monorepo-federation': 18
};
...
class DraftAudioPlayer {
    ...
    /** Auto-inject listen button on chapter pages that have audio. */
    setupChapterPage() {
        if (document.body.classList.contains('audio-player-page')) return;

        const chapterId = document.body.dataset.chapter;
        if (!chapterId) return;

        const audioIndex = CHAPTER_AUDIO_MAP[chapterId];
        ...
        btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg> Listen to this chapter';
        readTime.after(btn);

        btn.onclick = () => {
            const title = document.querySelector('h1')?.textContent || chapterId;
            this.playTrack(audioIndex, title, 'Draft Audiobook');
        };
    }
